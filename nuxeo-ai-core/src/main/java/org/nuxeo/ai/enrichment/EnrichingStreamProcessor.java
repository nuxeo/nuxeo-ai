        ...
        protected Collection<AIMetadata> callProvider(Record record, Callable<Collection<AIMetadata>> callable) {
            ...
            try {
                if (cfg != null && !cfg.isBlank()) {
                    int parsed = Integer.parseInt(cfg.trim());
                    if (parsed < 1) {
                        log.warn("{} must be >= 1 (got {}), falling back to {}s",
                                CALL_TIMEOUT_SECONDS_PROP, parsed, DEFAULT_CALL_TIMEOUT_SECONDS);
                    } else {
                        timeoutSeconds = parsed;
                    }
                }
            } catch (NumberFormatException e) {
                log.warn("Invalid value '{}' for {}, falling back to {}s", cfg, CALL_TIMEOUT_SECONDS_PROP,
