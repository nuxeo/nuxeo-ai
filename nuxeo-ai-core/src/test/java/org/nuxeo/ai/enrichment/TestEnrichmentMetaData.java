...
public class TestEnrichmentMetaData {
    ...
    public void testCacheKeys() throws IOException {
        ...
        PropertyNameType fileContentProp = new PropertyNameType(FILE_CONTENT, "img");
        blobTextFromDoc.getPropertyBlobs().get(fileContentProp).setDigest("47XX");
        assertEquals("testin47XX", EnrichmentUtils.makeKeyUsingBlobDigests(blobTextFromDoc, "testin"));
        ManagedBlob blob = blobTextFromDoc.getPropertyBlobs().get(fileContentProp);
