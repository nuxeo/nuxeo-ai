...
public class DatasetExportServiceImpl extends DefaultComponent implements DatasetExportService, DatasetStatsService {
    ...
    @Override
    public String export(CoreSession session, String nxql, Set<ModelProperty> inputProperties,
            Set<ModelProperty> outputProperties, int split) {
        return export(session, nxql, inputProperties, outputProperties, split, null);
    }

    @Override
    public String export(CoreSession session, String nxql, Set<ModelProperty> inputs, Set<ModelProperty> outputs,
        ...
        List<ModelProperty> featuresWithType = new ArrayList<>(inputs);
    ...
    @Override
    public Collection<Statistic> getStatistics(CoreSession session, String nxql, Set<ModelProperty> inputProperties,
            Set<ModelProperty> outputProperties) {
        Set<String> inputPropNames = inputProperties.stream().map(ModelProperty::getName).collect(Collectors.toSet());
        Set<String> outputPropNames = outputProperties.stream().map(ModelProperty::getName).collect(Collectors.toSet());
        validateParams(nxql, inputPropNames, outputPropNames);

        List<ModelProperty> featuresList = inputProperties.stream()
                                                          .map(p -> ExportHelper.addTypeIfNull(p))
                                                          .collect(Collectors.toList());
        featuresList.addAll(
                outputProperties.stream().map(p -> ExportHelper.addTypeIfNull(p)).collect(Collectors.toList()));
    ...
    protected void getValidStats(List<ModelProperty> featuresWithType, List<Statistic> stats, NxQueryBuilder qb) {
        for (ModelProperty prop : featuresWithType) {
     ...
     */
    @SuppressWarnings("unchecked")
    protected Long getOverallStats(List<ModelProperty> featuresWithType, List<Statistic> stats, NxQueryBuilder qb) {
    ...
    protected String notNullNxql(String nxql, List<ModelProperty> featuresWithType) {
