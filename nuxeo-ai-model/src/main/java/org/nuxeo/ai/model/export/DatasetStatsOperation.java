...
public class DatasetStatsOperation {
    ...
    public Blob run() throws IOException {
        Set<ModelProperty> inputProp;
        if (inputs != null) {
            inputProp = inputs.stream().map(p -> new ModelProperty(p, null)).collect(Collectors.toSet());
        ...
        Set<ModelProperty> outputProp;
        if (outputs != null) {
            outputProp = outputs.stream().map(p -> new ModelProperty(p, null)).collect(Collectors.toSet());
